...
from intelligence.canonical_feature_registry import FEATURE_COLS_V22, FEATURE_COLS_V30, FEATURE_COLS_V42, select_feature_frame
...
def main():
    ...
    # We use V4.2 columns for data loading to be safe, or just read all
    # But to match logic, let's just use the columns we expect
    feature_cols_load = FEATURE_COLS_V42
    
    # Check header first
    header = pd.read_csv(args.data, nrows=0).columns
    use_cols = [c for c in feature_cols_load if c in header]
    
    if len(use_cols) < len(feature_cols_load):
        print(f"Warning: {len(feature_cols_load) - len(use_cols)} features missing in CSV. Using subsets.")
        
    df = pd.read_csv(args.data, nrows=args.samples + SEQ_LEN + 100, usecols=lambda c: c in use_cols)
    
    # Pad missing columns with 0
    for c in feature_cols_load:
        if c not in df.columns:
            df[c] = 0.0
            
    X_raw, _, _, _, _ = (lambda x: (x, 0, 0, 0, 0))(df[feature_cols_load].values.astype(np.float32))
