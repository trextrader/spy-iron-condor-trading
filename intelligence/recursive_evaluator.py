...
def evaluate_predicates_recursive(
    ...
    for step in range(recursion_depth):
        ...
        if cmp_mask_r.any():
            ...
            # Use masked updates instead of dense 'where' to save memory?
            # actually 'where' allocates a new tensor.
            # in-place logic:
            
            # ADD (1)
            mask_add = (op_v == 1)
            if mask_add.any():
                rhs = torch.where(mask_add, rhs + v2, rhs)
                
            # SUB (2)
            mask_sub = (op_v == 2)
            if mask_sub.any():
                rhs = torch.where(mask_sub, rhs - v2, rhs)
                
            # MUL (3)
            mask_mul = (op_v == 3)
            if mask_mul.any():
                rhs = torch.where(mask_mul, rhs * v2, rhs)
