...
def build_raw_pivot_df(df: pd.DataFrame, left: int, right: int) -> pd.DataFrame:
...
def build_bar_level_dataset(
    ...
    """
    Creates one row per original bar and attaches:
    - pivot flags
    - last confirmed strong pivot info
    - active fib segment info based on most recent completed leg
    """
    out = df.copy()
    ...
    if not pivots.empty:
        ...
        low_mask = pivots["pivot_type"] == "LOW"

        out.loc[pivots.loc[high_mask, "pivot_index"].to_numpy(), "strong_pivot_high"] = 1
...
def plot_dataset_with_pivots_and_fibs(
    ...
    # Plot pivots within window
    if not pivots.empty:
        piv = pivots[(pivots["pivot_index"] >= start_global_idx)].copy()
        if not piv.empty:
            px = piv["pivot_index"].to_numpy() - start_global_idx
            py = piv["pivot_price"].to_numpy()
            high_mask = piv["pivot_type"] == "HIGH"
            low_mask = piv["pivot_type"] == "LOW"

            ax.scatter(px[high_mask], py[high_mask], marker="v", s=60, label="Strong Pivot High")
            ax.scatter(px[low_mask], py[low_mask], marker="^", s=60, label="Strong Pivot Low")

            # connect alternating pivots
            ax.plot(px, py, linewidth=1.2, alpha=0.8, label="Pivot Structure")

    # Plot fib segments for most recent legs
    if not fib_segments.empty:
        ...
        colors = [
            "#888888", "#d62728", "#2ca02c", "#ff7f0e", "#1f77b4", "#17becf",
            "#9467bd", "#8c564b", "#e377c2", "#7f7f7f"
        ]

        for s_i, seg in enumerate(segs.itertuples(index=False)):
            ...
            for col in segs.columns:
                if col.startswith("fib_"):
                    y = getattr(seg, col)
                    ax.hlines(y, xmin=x0, xmax=len(df_plot) - 1, linewidth=0.8, linestyles="dashed", alpha=0.5)
...
def process_one_file(input_csv: Path, output_root: Path) -> None:
    ...
    raw_pivots = build_raw_pivot_df(df, STRONG_LEFT, STRONG_RIGHT)
    strong_pivots = reduce_to_alternating_structure(raw_pivots)
    fib_segments = build_fib_segments(strong_pivots, FIB_LEVELS)
    bar_level = build_bar_level_dataset(df, strong_pivots, fib_segments, FIB_LEVELS)

    year_tf_dir = output_root / year / tf_key
    year_tf_dir.mkdir(parents=True, exist_ok=True)

    raw_pivots.to_csv(year_tf_dir / f"{tf_key}_{year}_raw_strong_pivots.csv", index=False)
    ...
    if MAKE_PLOTS:
        plot_dataset_with_pivots_and_fibs(
            df=df,
            pivots=strong_pivots,
            fib_segments=fib_segments,
            title=f"{tf_key.upper()} {year} Strong Pivots + Fib Segments",
            out_png=year_tf_dir / f"{tf_key}_{year}_strong_pivots_fibs.png",
        )

    print(
        f"[OK] {input_csv} | rows={len(df)} | raw_pivots={len(raw_pivots)} | "
        f"alt_pivots={len(strong_pivots)} | fib_segments={len(fib_segments)}"
    )
