...
CONFIG = {
    ...
    "margin_pct":         0.2000,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.2491,      # If pct_spot: margin = spot * this * 100

    # ── Exit: Stop-Loss ───────────────────────────────────────────────────
    "stop_loss_mult":     0.0000,   # Close when loss >= N * |credit| * qty * 100
    "stop_loss_dollar":   865.0000,  # Hard dollar cap per trade (None = use multiplier only)
    ...
    "put_offset_pct":     1.3000,  # % OTM for put strike  (None = neural model)
    "spread_width":       18.5000,  # Spread width points   (None = neural model / N/A)
    ...
    "short_delta":        0.2473,  # Target |delta| for short (selling) strikes
