"""
reverse_jade_lizard.py — Reverse Jade Lizard
===============================================
...
WHAT IT IS:
    Short call + bull put spread. Mirror image of jade lizard.
...
RISK PROFILE:
    Risk:   Upside risk from naked short call; downside defined by put spread.
    Reward: Limited to total credit received.

WHEN IT FIRES (predicate atoms):
    reversal_bear & ivr_high & rsi_overbought & friction_ok
...
CONFIG = {
    # ── Identity ──────────────────────────────────────────────────────────
    "template_id":        "reverse_jade_lizard",
    ...
    "stop_loss_dollar":   1325.0000,  # Hard dollar cap per trade (None = use multiplier only)

    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      425.0000,    # Dollar target (None = use 50% of credit)
    ...
    "call_offset_pct":    1.2865,  # % OTM for call strike (None = neural model)
    ...
    "short_delta":        0.3176,  # Target |delta| for short (selling) strikes
    "wing_delta":         0.0787,  # Target |delta| for long wing strikes (None = N/A)
