"""
covered_short_strangle.py — Covered Short Strangle
=====================================================
...
WHAT IT IS:
    Stock + short OTM strangle. Less risk than covered straddle.
...
RISK PROFILE:
    Risk:   Large risk, partially hedged by stock.
    Reward: Premium from both options.

WHEN IT FIRES (predicate atoms):
    ivr_high & consol_high & (trend_bull | trend_weak) & friction_ok
...
CONFIG = {
    # ── Identity ──────────────────────────────────────────────────────────
    "template_id":        "covered_short_strangle",
    ...
    "margin_spot_pct":    0.1034,      # If pct_spot: margin = spot * this * 100
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      740.0000,    # Dollar target (None = use 50% of credit)
    ...
    "call_offset_pct":    2.3034,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     2.1806,  # % OTM for put strike  (None = neural model)
    "spread_width":       18.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         7,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.0859,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.3332,  # Target |delta| for short (selling) strikes
