...
CONFIG = {
    ...
    "margin_pct":         0.3828,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.1655,      # If pct_spot: margin = spot * this * 100
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      1660.0000,    # Dollar target (None = use 50% of credit)
    ...
    "put_offset_pct":     1.4705,  # % OTM for put strike  (None = neural model)
    "spread_width":       10.5000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         10,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.0753,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.1628,  # Target |delta| for short (selling) strikes
    "wing_delta":         0.1938,  # Target |delta| for long wing strikes (None = N/A)
