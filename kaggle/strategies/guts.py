...
CONFIG = {
    ...
    "margin_pct":         0.4361,        # Max % of buying power to deploy per trade
    ...
    "call_offset_pct":    1.1000,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     1.1000,  # % OTM for put strike  (None = neural model)
    "spread_width":       9.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         17,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.1917,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.2686,  # Target |delta| for short (selling) strikes
