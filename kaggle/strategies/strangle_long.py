...
CONFIG = {
    ...
    "margin_pct":         0.4000,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.1500,      # If pct_spot: margin = spot * this * 100
    ...
    "call_offset_pct":    2.1586,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     2.1905,  # % OTM for put strike  (None = neural model)
    ...
    "short_delta":        0.4146,  # Target |delta| for short (selling) strikes
