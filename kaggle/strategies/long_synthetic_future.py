...
CONFIG = {
    ...
    "margin_pct":         0.4164,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    -0.0262,      # If pct_spot: margin = spot * this * 100

    # ── Exit: Stop-Loss ───────────────────────────────────────────────────
    "stop_loss_mult":     10.0000,   # Close when loss >= N * |credit| * qty * 100
    "stop_loss_dollar":   975.0000,  # Hard dollar cap per trade (None = use multiplier only)
    ...
    "call_offset_pct":    0.0000,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     -0.0866,  # % OTM for put strike  (None = neural model)
    "spread_width":       20.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         20,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.1879,  # Max bid-ask ratio per leg
