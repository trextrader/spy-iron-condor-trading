"""
inverse_iron_condor.py — Inverse Iron Condor
===============================================
...
WHAT IT IS:
    Wider inverse butterfly. Buy inner strangle, sell outer strangle. Profits from large move.
...
RISK PROFILE:
    ...
    Reward: Defined: max profit = width − debit.
...
CONFIG = {
    # ── Identity ──────────────────────────────────────────────────────────
    "template_id":        "inverse_iron_condor",
    ...
    "margin_pct":         0.5802,        # Max % of buying power to deploy per trade
    "margin_type":        "width",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.3925,      # If pct_spot: margin = spot * this * 100

    # ── Exit: Stop-Loss ───────────────────────────────────────────────────
    "stop_loss_mult":     10.0000,   # Close when loss >= N * |credit| * qty * 100
    "stop_loss_dollar":   460.0000,  # Hard dollar cap per trade (None = use multiplier only)

    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      550.0000,    # Dollar target (None = use 50% of credit)
    ...
    "call_offset_pct":    1.4098,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     1.5847,  # % OTM for put strike  (None = neural model)
    "spread_width":       20.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         8,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.0375,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.3488,  # Target |delta| for short (selling) strikes
