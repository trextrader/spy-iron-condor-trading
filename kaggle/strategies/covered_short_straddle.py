"""
covered_short_straddle.py — Covered Short Straddle
=====================================================
...
WHAT IT IS:
    Stock + short ATM straddle. Income with underlying protection on downside.
...
RISK PROFILE:
    Risk:   Large risk both directions, partially hedged by stock.
    Reward: Premium from both options + stock appreciation to call strike.

WHEN IT FIRES (predicate atoms):
    ivr_high & consol_vhigh & trend_bull & regime_stable & gap_low & friction_ok
...
CONFIG = {
    # ── Identity ──────────────────────────────────────────────────────────
    "template_id":        "covered_short_straddle",
    ...
    "margin_pct":         0.6714,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.0375,      # If pct_spot: margin = spot * this * 100

    # ── Exit: Stop-Loss ───────────────────────────────────────────────────
    "stop_loss_mult":     0.0000,   # Close when loss >= N * |credit| * qty * 100
    "stop_loss_dollar":   440.0000,  # Hard dollar cap per trade (None = use multiplier only)

    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      2535.0000,    # Dollar target (None = use 50% of credit)
    ...
    "cooldown_bars":      19,           # Min bars between trades

    # ── Entry: Structure (overrides neural model when not None) ──────────────
    "call_offset_pct":    -0.0639,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     0.0459,  # % OTM for put strike  (None = neural model)
    "spread_width":       6.5000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         22,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.3087,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.1582,  # Target |delta| for short (selling) strikes
