...
CONFIG = {
    ...
    "margin_pct":         0.7040,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.1035,      # If pct_spot: margin = spot * this * 100
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      1400.0000,    # Dollar target (None = use 50% of credit)
    ...
    "cooldown_bars":      22,           # Min bars between trades

    # ── Entry: Structure (overrides neural model when not None) ──────────────
    "call_offset_pct":    0.0320,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     -0.0022,  # % OTM for put strike  (None = neural model)
    "spread_width":       13.5000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         19,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.2250,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.1829,  # Target |delta| for short (selling) strikes
