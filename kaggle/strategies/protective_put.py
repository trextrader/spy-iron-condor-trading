"""
protective_put.py — Protective Put
=====================================
...
WHAT IT IS:
    Portfolio insurance. Buy put to protect long stock position from crash. Pays premium for protection.

LEG STRUCTURE:
#   Leg 1: LONG P rank0

RISK PROFILE:
    Risk:   Limited to premium paid (cost of insurance).
    Reward: Profits from downside crash.

WHEN IT FIRES (predicate atoms):
    gap_high | (chaos_high & trend_bull)
...
CONFIG = {
    ...
    "margin_pct":         0.6508,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.0930,      # If pct_spot: margin = spot * this * 100
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      1100.0000,    # Dollar target (None = use 50% of credit)
    ...
    "put_offset_pct":     0.9000,  # % OTM for put strike  (None = neural model)
    "spread_width":       16.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         9,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.1931,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.4093,  # Target |delta| for short (selling) strikes
