...
DIRECTION: Bullish/Hedge

WHAT IT IS:
    Stock + long put + short call. Caps upside and downside. Zero-cost if strikes balanced.
...
RISK PROFILE:
    Risk:   Defined: max loss = stock − put strike + net debit.
    Reward: Capped at call strike − stock + net credit.

WHEN IT FIRES (predicate atoms):
    trend_bull & (gap_high | chaos_high) & friction_ok
...
CONFIG = {
    ...
    # ── Sizing ────────────────────────────────────────────────────────────
    "max_contracts":      14,          # Max contracts per trade
    "margin_pct":         0.4534,        # Max % of buying power to deploy per trade
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      2300.0000,    # Dollar target (None = use 50% of credit)
    ...
    "short_delta":        0.3551,  # Target |delta| for short (selling) strikes
