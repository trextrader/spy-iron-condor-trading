"""
put_broken_wing.py — Put Broken Wing
=======================================
...
WHAT IT IS:
    Skip-strike put butterfly. Asymmetric — skewed to reduce downside risk. Slight bull bias.
...
RISK PROFILE:
    Risk:   Asymmetric: reduced on the bullish side.
    Reward: Max profit at body strike.

WHEN IT FIRES (predicate atoms):
    (trend_bull | reversal_bull) & ~ivr_high & consol_high & friction_ok
...
CONFIG = {
    ...
    # ── Sizing ────────────────────────────────────────────────────────────
    "max_contracts":      11,          # Max contracts per trade
    "margin_pct":         0.3927,        # Max % of buying power to deploy per trade
    "margin_type":        "width",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.5627,      # If pct_spot: margin = spot * this * 100
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      1375.0000,    # Dollar target (None = use 50% of credit)
    ...
    # ── Position Limits ───────────────────────────────────────────────────
    "max_positions":      19,           # Max concurrent open positions for this strategy
    ...
    "put_offset_pct":     1.4853,  # % OTM for put strike  (None = neural model)
    "spread_width":       6.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         8,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.2440,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.1653,  # Target |delta| for short (selling) strikes
    "wing_delta":         0.1243,  # Target |delta| for long wing strikes (None = N/A)
