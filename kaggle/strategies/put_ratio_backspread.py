...
CONFIG = {
    ...
    "margin_pct":         0.6160,        # Max % of buying power to deploy per trade
    "margin_type":        "pct_spot",   # "width" (spread) or "pct_spot" (naked)
    "margin_spot_pct":    0.0643,      # If pct_spot: margin = spot * this * 100
    ...
    # ── Exit: Profit Target ───────────────────────────────────────────────
    "profit_target":      1425.0000,    # Dollar target (None = use 50% of credit)
    ...
    "put_offset_pct":     1.4864,  # % OTM for put strike  (None = neural model)
    "spread_width":       8.0000,  # Spread width points   (None = neural model / N/A)
    ...
    "short_delta":        0.2715,  # Target |delta| for short (selling) strikes
    "wing_delta":         0.3875,  # Target |delta| for long wing strikes (None = N/A)
