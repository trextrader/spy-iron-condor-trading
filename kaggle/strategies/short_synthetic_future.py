"""
short_synthetic_future.py — Short Synthetic Future
=====================================================
...
WHAT IT IS:
    Buy put + sell call at same strike. Replicates short selling 100 shares.
...
WHEN IT FIRES (predicate atoms):
    trend_bear & adx_vstrong & friction_ok
...
CONFIG = {
    # ── Identity ──────────────────────────────────────────────────────────
    "template_id":        "short_synthetic_future",
    ...
    "margin_spot_pct":    0.1060,      # If pct_spot: margin = spot * this * 100
    ...
    "call_offset_pct":    0.0957,  # % OTM for call strike (None = neural model)
    "put_offset_pct":     0.0894,  # % OTM for put strike  (None = neural model)
    "spread_width":       20.0000,  # Spread width points   (None = neural model / N/A)
    "target_dte":         12,  # Target DTE at entry   (None = neural model)

    # ── Entry: Quality Filter ─────────────────────────────────────────────────
    "max_leg_spread":     0.2982,  # Max bid-ask ratio per leg

    # ── Entry: Delta Targets (template path) ────────────────────────────────
    "short_delta":        0.1501,  # Target |delta| for short (selling) strikes
