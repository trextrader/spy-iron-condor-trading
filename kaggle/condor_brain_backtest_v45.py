"""
CondorBrain v4.5 Backtest — Multi-Position Portfolio Engine
============================================================
Evolved from V2.2 to V4.5:
  - Multi-position capital-gated entry (Level 3, 2:1 leverage, 40% max deploy)
  - Per-gate granular debug counters (12 distinct failure modes)
  - OPEN/CLOSE event rows for state-action trajectory export
  - Deep predicate audit trail: every entry decision is fully decomposed
  - Width-relative credit floor (max(0.10, 5% of width)) replaces flat $0.10
  - Win-probability pool filter (75% threshold) before capital allocation
  - Halt-on-inconsistency flag: stops sim and logs root cause if sizing anomaly
  - Starting equity: $100,000 | Leverage: 2:1 | Max options deploy: $80,000

Architecture: single-strategy (Iron Condor) with StrategyRegistry stub.
Phase 4 CapitalConstraintEngine is inline; external module wired in Phase 4+.

Usage:
1. Upload to Kaggle/Lightning AI.
2. Ensure model checkpoint and data path are configured.
...
# Account: Options Level 3 | $100k starting | 2:1 leverage | 40% max deploy
# =============================================================================
STARTING_EQUITY           = 100_000.0   # Account starting cash equity
LEVERAGE_FACTOR           = 2.0         # Level 3 margin account: 2:1 leverage
BUYING_POWER              = STARTING_EQUITY * LEVERAGE_FACTOR  # $200,000
MAX_OPTIONS_ALLOC_PCT     = 0.40        # Max 40% of buying power in open options
MAX_DEPLOY                = BUYING_POWER * MAX_OPTIONS_ALLOC_PCT  # $80,000 ceiling

# Per-trade collateral bounds (as pct of buying power)
MIN_COLLATERAL_PCT        = 0.005       # Floor: $1,000 min collateral per trade
MAX_COLLATERAL_PCT        = 0.15        # Cap: $30,000 max collateral per single trade

# Entry pool thresholds (multi-tier screening)
MIN_WIN_PROB_POOL         = 0.75        # Tier-1: include in pool if win_prob >= 75%
MIN_CREDIT_WIDTH_RATIO    = 0.05        # Credit must be >= 5% of spread width
# (e.g. $10 wide spread requires >= $0.50 credit — tighter than flat $0.10)
...
def run_backtest(df, rule_signals, model, feature_cols, device, ruleset=None, model_path=None, data_path=None, norm_stats=None,
    ...
    def _deployed_margin(trades):
        """Total collateral at risk across all open (non-closed) trades."""
        total = 0.0
        for t in trades:
            if not t.is_closed:
                w = float(t.legs.get('width', 0.0))
                total += max(0.0, (w - t.net_credit)) * IC_MULTIPLIER * t.qty
        return total

    def _can_allocate(trades, new_margin):
        """True if adding new_margin keeps total deployed <= MAX_DEPLOY."""
        return (_deployed_margin(trades) + new_margin) <= MAX_DEPLOY

    def _entry_audit_log(bar_idx, ts, spot, pol, gate, reason, extra=None):
        """
        Emit a structured entry audit line to console + bar trace.
        gate: 'PASS' | 'SKIP' | 'HALT'
        reason: short string identifying which check decided the outcome
        extra: dict of additional context (leg strikes, credit, sizing, etc.)
        """
        rec = {
            'action':     'ENTRY_AUDIT',
            'gate':       gate,
            ...
            # Policy vector decomposition
            'pol_call_off_raw':  round(float(pol[0]), 6),
            'pol_put_off_raw':   round(float(pol[1]), 6),
            'pol_width_raw':     round(float(pol[2]), 6),
            'pol_dte_raw':       round(float(pol[3]), 6),
            'pol_pop_logit':     round(float(pol[4]), 6),
            'pol_pop_prob':      round(float(_sigmoid(pol[4])), 4),
            'pol_conf_logit':    round(float(pol[7]), 6),
            'pol_conf_prob':     round(float(_sigmoid(pol[7])), 4),
            'pol_entry_logit':   round(float(pol[8]), 6),
            # Decoded parameters
            'call_off_pct':      round(float(pol[0]) * 5.0, 4),
            'put_off_pct':       round(float(pol[1]) * 5.0, 4),
            'width_decoded':     round(float(pol[2]) * 10.0, 4),
            'dte_decoded':       round(float(pol[3]) * 45.0, 4),
        }
        if extra:
            rec.update(extra)
        if BAR_TRACE_ENABLED:
            _append_jsonl(BAR_TRACE_PATH, rec)
        if DEBUG_INFERENCE and gate in ('PASS', 'HALT'):
            print(f"  [ENTRY_AUDIT:{gate}] bar={bar_idx} ts={ts} reason={reason} "
                  f"spot={spot:.2f} entry_logit={pol[8]:.4f} "
    ...
    for i in tqdm(range(sim_start_idx, num_bars), desc="Simulating"):
         ...
         # =====================================================================
         # 5. ENTRY LOGIC — V4.5 (MULTI-POSITION, CAPITAL-GATED, AUDITABLE)
         # =====================================================================
         # Allows multiple simultaneous positions up to MAX_DEPLOY ceiling.
         # Each gate failure is counted distinctly for full observability.
         # Every entry decision emits an audit record to bar_trace.jsonl.
         # On halt-worthy inconsistencies, simulation stops with full dump.
         #
         # Capital model:
         #   equity         = current cash equity ($100k start)
         #   BUYING_POWER   = equity × LEVERAGE_FACTOR (2×)
         #   MAX_DEPLOY     = BUYING_POWER × MAX_OPTIONS_ALLOC_PCT (40%)
         #   trade_margin   = (width − credit) × 100 × qty
         #   Entry allowed iff total_deployed + trade_margin <= MAX_DEPLOY
         # =====================================================================
         ...
         if not np.all(np.isfinite(pol)):
             ...
             # (C) Neural gate checks — each gate tracked separately
                 ...
                 required_cols = ['option_symbol', 'call_put', 'strike']
                 missing_chain = [c for c in required_cols if c not in chain_with_prices.columns]
                 if missing_chain:
                     run_backtest._entry_dbg['missing_chain_cols'] += 1
                     _entry_audit_log(i, ts, spot, pol, 'SKIP', f'chain_missing_cols:{missing_chain}')
                     ...
                     chain_te = None
                     if 'te' in chain_with_prices.columns:
                         te_vals = pd.to_numeric(chain_with_prices['te'], errors='coerce').dropna()
                         if len(te_vals) > 0:
                     ...
                     if _strat_cfg.get("margin_type") == "pct_spot":
                         ...
                         estimated_margin = spot * 0.02 * IC_MULTIPLIER * _est_qty
                     else:
                         estimated_margin = max(0.0, width) * IC_MULTIPLIER * _est_qty


                     # (F) Capital constraint check — update buying power with current equity
                     live_buying_power = equity * LEVERAGE_FACTOR
                     live_max_deploy   = live_buying_power * MAX_OPTIONS_ALLOC_PCT
                     currently_deployed = _deployed_margin(open_trades)
                     ...
                     if not (_collateral_floor <= estimated_margin <= max_collateral * 2.0):
                         ...
                         if verbose_sim:
                             print(f"  -- SKIP collateral_oob: est=${estimated_margin:,.0f} floor=${_collateral_floor:,.0f} cap=${max_collateral*2:,.0f} qty={_est_qty}")
                         run_backtest._entry_dbg['collateral_oob'] += 1
                         _entry_audit_log(i, ts, spot, pol, 'SKIP',
                             f'collateral_oob:{estimated_margin:.0f} '
                             f'not_in[{min_collateral:.0f},{max_collateral*1.5:.0f}]',
                         ...
                         _entry_audit_log(i, ts, spot, pol, 'SKIP',
                             ...
                             f'new={estimated_margin:.0f}>'
                             f'max={live_max_deploy:.0f}',
                             extra={'deployed': round(currently_deployed, 2),
                                     ...
                                     if (currently_deployed_exact + trade_margin) > live_max_deploy:
                                         ...
                                         if int(filled_qty) != expected_qty:
                                             audit_msg = (
                                                 ...
                                                 f"expected_qty={expected_qty} filled_qty={filled_qty} "
                                                 f"trade_margin={trade_margin:.2f} credit={net_credit_val:.4f} "
                                                 f"width={spread_width:.2f} legs={legs}"
                                             )
                                             _append_jsonl(BAR_TRACE_PATH, {
                                                 'action': 'SIZING_INCONSISTENCY', 'bar_idx': i,
                                                 'ts': str(ts), 'expected_qty': expected_qty,
                                                 'filled_qty': int(filled_qty), 'trade_margin': trade_margin,
                                             })
                                             print(audit_msg)
                                         ...
                                         new_trade = Trade(
                                             ...
                                             scores={
                                                 ...
                                                 'conf_prob':        round(conf_prob, 4),
                                                 'call_off_pct':     round(call_off, 4),
                                                 'put_off_pct':      round(put_off, 4),
                                                 'width':            round(spread_width, 4),
                                                 'dte':              round(dte, 2),
                                                 'gross_credit':     round(fill_details.get('gross_credit', 0), 4),
                                                 'slippage':         round(fill_details.get('total_slippage',
                                                                       fill_details.get('slippage', 0)), 4),
                                                 'short_call_delta': legs.get('short_call_delta'),
                                                 'short_put_delta':  legs.get('short_put_delta'),
                                                 'exec_reality':     exec_engine is not None,
                                                 'trade_margin':     round(trade_margin, 2),
                                                 'deployed_before':  round(currently_deployed_exact, 2),
                                                 'live_max_deploy':  round(live_max_deploy, 2),
                                             }
                                         )
                                         ...
                                         closed_trades.append({
                                             ...
                                             'dt':             str(ts),
                                             'spot':           round(spot, 4),
                                             'entry_credit':   round(net_credit_val, 4),
                                             'max_loss':       round(max_loss, 2),
                                             'trade_margin':   round(trade_margin, 2),
                                             'dte_entry':      round(dte, 2),
                                             'short_call':     legs.get('short_call'),
                                             'long_call':      legs.get('long_call'),
                                             'short_put':      legs.get('short_put'),
                                             'long_put':       legs.get('long_put'),
                                             'width':          round(spread_width, 4),
                                             'entry_logit':    round(entry_logit, 6),
                                             'pop_prob':       round(pop_prob, 4),
                                             'conf_prob':      round(conf_prob, 4),
                                         ...
                                         _entry_audit_log(i, ts, spot, pol, 'PASS', 'trade_opened',
                                             extra={
                                                 'trade_id':   trade_id,
                                                 'credit':     round(net_credit_val, 4),
                                                 'width':      round(spread_width, 4),
                                                 'margin':     round(trade_margin, 2),
    ...
    if hasattr(run_backtest, '_entry_dbg'):
        ...
        bars   = max(d['bars_seen'], 1)
        opens  = d['success']
        print("\n" + "="*80)
        print("  V4.5 ENTRY GATE SUMMARY")
        print("="*80)
        print(f"  {'bars_seen':<28s}: {d['bars_seen']:>8,}")
        print(f"  {'cooldown_block':<28s}: {d['cooldown_block']:>8,}  ({d['cooldown_block']/bars*100:.1f}%)")
        print(f"  {'capital_block':<28s}: {d['capital_block']:>8,}  ({d['capital_block']/bars*100:.1f}%)")
        print(f"  {'policy_nan_block':<28s}: {d['policy_nan_block']:>8,}")
    ...
    if hasattr(run_backtest, '_credit_samples') and run_backtest._credit_samples:
        ...
        for idx, cs in enumerate(run_backtest._credit_samples):
            ...
            print(f"       Calls: S{cs.get('s_call',0):.0f}(bid ${cs.get('s_call_bid',0):.2f}) -> "
                  f"L{cs.get('l_call',0):.0f}(ask ${cs.get('l_call_ask',0):.2f})")
            print(f"       Puts:  S{cs.get('s_put',0):.0f}(bid ${cs.get('s_put_bid',0):.2f}) -> "
                  f"L{cs.get('l_put',0):.0f}(ask ${cs.get('l_put_ask',0):.2f})")
    ...
    if getattr(args, "optimize", False):
        ...
        if _opt_mode == "bayes":
            ...
            if _autoall:
                ...
                print(f"[autoall] {'═'*68}")
                # Load registry to show final applied/skipped status
