...
def simulate_broker_execution(
    ...
    max_lag_bars: int = 1,
    atomicity_threshold: float = 0.05
) -> tuple:
    """
    Simulates reality-layer execution logic (Phase 5).
    
    Args:
        legs: The Iron Condor leg structure
        target_qty: Number of contracts
        quote_chain: Current option quotes
        slippage_rate: Base slippage per contract
        max_lag_bars: Maximum simulated bar delay for full fill
        atomicity_threshold: Max allowed price deviation across legs before trade is 'broken'
        
    Returns:
        (filled_qty, effective_credit, is_atomic, details)
    """
    ...
    if not all(s in current_prices for s in syms):
        return 0, 0.0, False, {"reason": "missing_legs"}
        
    # 2. Leg Symmetry/Atomicity Check
    # In reality, legs aren't filled at the same microsecond.
    # We simulate this by checking if any leg moved significantly since decision.
    # (Simplified: we use the mid-price but apply a 'Sync Penalty')
    base_credit = (current_prices[legs.short_call.symbol] + current_prices[legs.short_put.symbol]) - \
                  (current_prices[legs.long_call.symbol] + current_prices[legs.long_put.symbol])
    
    # Simulate a 'Broken Wing' risk if spread is too wide or liquidity is low
    # For now, we just apply a multiplier to slippage based on 'lag'
    lag_slippage = slippage_rate * (1.0 + 0.5 * max_lag_bars)
    
    # 3. Final Execution Math
    effective_credit = base_credit - (lag_slippage * 4) # 4 legs
    
    return target_qty, float(effective_credit), True, {"base_credit": base_credit, "slippage": lag_slippage}
...
def load_intraday_options(file_path: str, start_date=None, end_date=None) -> dict:
    ...
    use_cols = [
        'timestamp', 'symbol', 'expiration', 'strike', 'call_put', 
    ...
    # Date parsing (deferred for speed)
...
class IronCondorStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.losses = 0
        self.trades = 0
        self.peak = self.r_cfg.backtest_cash if self.r_cfg is not None else 100000.0
        self.drawdowns = []
        self.equity_series = []
        self.trade_log = []
    ...
    def next(self):
        ...
        cash_base = self.r_cfg.backtest_cash if self.r_cfg else 100000.0
        current_equity = cash_base + self.pnl
        ...
        if not self.is_intraday:
            if isinstance(chain_records, pd.DataFrame):
                ...
                for r in chain_records.itertuples():
                    ...
                    if k in seen_contracts: continue
                    seen_contracts.add(k)
                    quote_chain.append(OptionQuote(
                        symbol=getattr(r, 'option_symbol'),
                        expiration=expiration, strike=strike, is_call=is_call,
                        bid=getattr(r, 'bid'), ask=getattr(r, 'ask'), mid=getattr(r, 'last_price'),
                        delta=getattr(r, 'delta'), iv=getattr(r, 'implied_volatility'),
                        gamma=getattr(r, 'gamma', 0.0), vega=getattr(r, 'vega', 0.0), theta=getattr(r, 'theta', 0.0)
                    ))
            else:
                seen_contracts = set()
                for r in chain_records:
                    if abs(r.get('delta', 0) or 0) < 0.005: continue
                    strike = r['strike']
                    ...
                    quote_chain.append(OptionQuote(
                        ...
                        expiration=expiration, strike=strike, is_call=is_call,
                        bid=r['bid'], ask=r['ask'], mid=r['last_price'],
                        delta=r['delta'], iv=r['implied_volatility'],
                        gamma=r.get('gamma', 0.0) or 0.0, vega=r.get('vega', 0.0) or 0.0, theta=r.get('theta', 0.0) or 0.0
                    ))
        ...
        if self.bars_since_trade < 100:
            if self.verbose: print(f"  [EntryFail] Cooling period active at {dt_now}")
            return

        # Indicators
        price_history = self.data.close.get(ago=0, size=21)
        price_series = pd.Series(price_history) if price_history else pd.Series()
        realized_vol = compute_realized_vol(price_series, window=20)
        if not hasattr(self, 'vol_history'): self.vol_history = []
        ...
        if ivr < self.s_cfg.iv_rank_min or vix > self.s_cfg.vix_threshold:
            if self.verbose: print(f"  [EntryFail] IV filter: IVR {ivr:.2f} < {self.s_cfg.iv_rank_min} or VIX {vix:.2f} > {self.s_cfg.vix_threshold} at {dt_now}")
            return
        ...
        if len(self.equity_series) < 60:
            if self.verbose: print(f"  [EntryFail] Warmup period active ({len(self.equity_series)} < 60) at {dt_now}")
            return

        # Regime
        current_regime = MarketRegime.LOW_VOL_RANGE 
        if vix > 35.0: current_regime = MarketRegime.CRASH_MODE
        elif self.adx[0] > 25.0: current_regime = MarketRegime.BULL_TREND if self.data.close[0] > self.sma[0] else MarketRegime.BEAR_TREND
        elif vix > 20.0: current_regime = MarketRegime.HIGH_VOL_RANGE
        
        base_width = self.s_cfg.wing_width_min
        if current_regime in [MarketRegime.HIGH_VOL_RANGE, MarketRegime.CRASH_MODE]: width = min(self.s_cfg.wing_width_max, base_width + self.s_cfg.wing_increment_high)
        elif current_regime in [MarketRegime.BULL_TREND, MarketRegime.BEAR_TREND]: width = min(self.s_cfg.wing_width_max, base_width + self.s_cfg.wing_increment_med)
        else: width = base_width

        condor = build_condor(quote_chain, self.s_cfg, date_now, self.data.close[0], width)
        if not condor:
            if self.verbose: print(f"  [EntryFail] build_condor failed at {dt_now}")
            return

        credit = calc_condor_credit(condor)
        required_ratio = self.s_cfg.min_credit_to_width
        if ivr < 20.0: required_ratio = max(0.12, required_ratio * 0.7)
        elif ivr > 50.0: required_ratio = min(0.35, required_ratio * 1.2)
        
        min_credit = width * required_ratio
        if credit < min_credit:
            if self.verbose: print(f"  [EntryFail] credit ${credit:.2f} < min_credit ${min_credit:.2f} at {dt_now}")
            return
        ...
        if self.neural_forecasts is not None and not self.neural_forecasts.empty:
            ...
            if 0 <= idx < len(self.neural_forecasts):
                ...
                mamba_conf = float(row['mamba_conf'])
                neural_forecast_data = {'model_backend': 'Mamba2-Batch', 'confidence': mamba_conf, 'prob_bull': float(row['mamba_bull']), 'prob_bear': float(row['mamba_bear']), 'prob_neutral': float(row['mamba_neutral'])}

        intent = TradeIntent(
            symbol=self.s_cfg.underlying, action="SELL_CONDOR", gaussian_confidence=mamba_conf,
            current_price=self.data.close[0], vix=vix, ivr=ivr, realized_vol=0.0, mtf_snapshot=mtf_snapshot,
            ...
            extras={
                ...
                'min_gaussian_confidence': 0.10,
                'fallback_total_qty': 2
            }
        )
        plan = benchmark_and_size(intent)
        if not plan.approved:
            if self.verbose: print(f"  [EntryFail] Sizing plan not approved: {plan.reason} at {dt_now}")
            return
        ...
        if not risk_approved:
            if self.verbose: print(f"  [EntryFail] RiskManager rejected: {risk_reason} at {dt_now}")
            return
        ...
        filled_qty, net_credit, is_atomic, exec_details = simulate_broker_execution(
            legs=condor, target_qty=quantity, quote_chain=quote_chain,
            slippage_rate=slippage_rate, max_lag_bars=getattr(self.r_cfg, "max_broker_lag_bars", 1)
        )
        if not is_atomic or filled_qty == 0:
            if self.verbose: print(f"  [EntryFail] Execution failed: {exec_details.get('reason', 'unknown')} at {dt_now}")
            return

        entry_commission = commission_rate * filled_qty * 4
        entry_ohlcv = {
        ...
        print(f"[Trade] Opened | Qty: {filled_qty} | Credit: ${credit:.2f} (net: ${net_credit:.2f})")
        self.active_position = PositionState(
            id=f"BT-{len(self.trade_log)+1}", legs=condor, open_time=dt_now,
            credit_received=net_credit, quantity=filled_qty, 
            adjustments_done={"call": 0, "put": 0},
            mtf_consensus=mu_mtf
        )
        self.active_position.metadata = {
            "ivr": ivr, "vix": vix, "width": width, 
            "gross_credit": credit, "net_credit": net_credit,
